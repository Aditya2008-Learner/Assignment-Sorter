...
class DatabaseRepo:
    ...
    def create_assignment(data: Dict[str, Any]) -> Dict[str, Any]:
        ...
        conn.commit()
        conn.close()
        return DatabaseRepo.get_assignment_by_id(assignment_id) # type: ignore
    ...
    def create_notebook(data: Dict[str, Any]) -> Dict[str, Any]:
        ...
        conn.commit()
        conn.close()
        return DatabaseRepo.get_notebook_by_id(notebook_id) # type: ignore
    ...
    def get_weakness_radar() -> Dict[str, Any]:
        ...
        for r in rows:
            ...
            for t in weak:
                if t not in topic_stats:
                    topic_stats[t] = {"weak": 0, "mastered": 0}
        ...
        for t, stats in topic_stats.items():
            ...
            mastery_pct = round((stats["mastered"] / total) * 100, 1) if total > 0 else 0
            topics_summary.append({
                "topic": t,
                "mastery_percentage": mastery_pct,
                "is_weak": mastery_pct < 65,
                "attempts": total
            })
        ...
        return {
            ...
            "topic_analytics": topics_summary
        }
    ...
    def get_distinct_metadata() -> Dict[str, List[str]]:
        ...
        cursor.execute("SELECT DISTINCT subject FROM assignments WHERE subject IS NOT NULL AND subject != '' UNION SELECT DISTINCT subject FROM pyq_papers WHERE subject != '' UNION SELECT DISTINCT subject FROM notebooks WHERE subject != '' UNION SELECT DISTINCT name FROM curriculum")
        subjects = [r[0] for r in cursor.fetchall()]
        
        cursor.execute("SELECT DISTINCT semester FROM assignments WHERE semester IS NOT NULL AND semester != '' UNION SELECT DISTINCT semester FROM pyq_papers WHERE semester != '' UNION SELECT DISTINCT semester FROM curriculum")
        semesters = sorted(list(set([str(r[0]) for r in cursor.fetchall()])))
    ...
    def seed_curriculum(courses: List[Dict[str, Any]]):
        ...
        for c in courses:
            ...
            """, (
                c["code"],
                c["name"],
                int(c.get("semester", c.get("semester_number", 1))),
                int(c.get("credits", 3)),
                c.get("category", "Professional Core"),
                c.get("type", "Core"),
                json.dumps(c.get("modules", [])),
                json.dumps(c.get("outcomes", [])),
                json.dumps(c.get("key_textbooks", [])),
                json.dumps(c.get("assignments", []))
            ))
        conn.commit()
        conn.close()
