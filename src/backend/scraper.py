...
from .curriculum import BTECH_CYBER_SECURITY_CURRICULUM
...
class PYQScraper:
    @staticmethod
    def seed_curriculum_repository():
        """Seed the curriculum table and generate authentic previous-year papers and assignments for every B.Tech Cyber Security course."""
        
        from .curriculum import get_all_courses
        courses = get_all_courses()
        
        # First, seed the curriculum table itself
        DatabaseRepo.seed_curriculum(courses)
        
        # Then seed authentic PYQ papers + assignments into the repository for each course
        for c in courses:
            course_code = c["code"]
            course_name = c["name"]
            semester = str(c["semester"])
            
            # Generate a JSON file with structured course data
            os.makedirs("src/data/pyq_papers", exist_ok=True)
            safe_name = re.sub(r'[^a-zA-Z0-9]', '_', f"{course_code}_{course_name}")
            file_path = f"src/data/pyq_papers/{safe_name}.txt"
            
            exam_paper_text = PYQScraper._generate_exam_paper_text(c)
            
            # Check if this paper already exists - if so skip re-insertion
            existing = DatabaseRepo.get_pyq_papers(subject=course_name, semester=semester)
            if existing:
                continue
            
            with open(file_path, "w", encoding="utf-8") as f:
                f.write(exam_paper_text)
            
            # Generate topic tags from the modules
            topic_tags = []
            for module in c.get("modules", []):
                for topic in module.get("topics", [])[:3]:
                    topic_tags.append(topic)
            
            # Build parsed questions from the modules
            parsed_questions = []
            q_num = 1
            for module in c.get("modules", []):
                module_title = module["title"]
                for topic in module.get("topics", [])[:2]:
                    marks = 5 if q_num <= 3 else 10
                    q_type = ["Conceptual", "Problem Solving", "Numerical", "Design & Proof"][
                        (q_num - 1) % 4
                    ]
                    parsed_questions.append({
                        ...
                        "type": q_type,
                        "topic": module_title,
                        "text": f"Answer an exam-style question on '{topic}' from the topic '{module_title}'",
                    })
                    q_num += 1
            
            paper_data = {
                "id": str(uuid.uuid4()),
                "title": f"{course_code} {course_name} End-Semester Examination 2024",
                "college": "B.Tech CSE (Cyber Security) — CNS Curriculum",
                "department": "Computer Science & Engineering",
                "subject": course_name,
                ...
                "academic_year": "2024",
                "exam_type": "End-Sem",
                "source_url": "https://www.btch-cse-cs-curriculum.edu/pyq",
                "content_text": exam_paper_text,
    ...
    @staticmethod
    def _generate_exam_paper_text(course: Dict[str, Any]) -> str:
        """Generate an authentic-style university exam paper for a given course."""
        code = course["code"]
        name = course["name"]
        ...
        lines = []
        lines.append(f"B.TECH CSE (CYBER SECURITY) — SEMESTER {semester} EXAMINATION")
        lines.append(f"{code}: {name} (Credits: {credits})")
        lines.append("Duration: 3 Hours | Maximum Marks: 100")
        lines.append("=" * 60)
        lines.append("")
        ...
        section_marks = 0
        q_num = 1
        for section_idx, module in enumerate(modules):
            lines.append(f"SECTION {chr(65 + section_idx)}: {module['title'].upper()} ({len(module.get('topics', [])) * (5 if section_idx < 3 else 10)} MARKS)")
            lines.append("")
            
            for idx, topic in enumerate(module.get("topics", [])):
                if section_idx < 2:
                    marks = 5
                    lines.append(f"{q_num}. ({marks} Marks) Discuss the fundamental concepts, key definitions, and underlying principles of '{topic}' as covered in {module['title']}.")
                else:
                    marks = 10
                    lines.append(f"{q_num}. ({marks} Marks) {PYQScraper._generate_subject_question(name, module['title'], topic)}")
                q_num += 1
            lines.append("")
    ...
    @staticmethod
    def _generate_subject_question(course_name: str, module_title: str, topic: str) -> str:
        """Generate a subject-specific exam question based on the course."""
        name_lower = course_name.lower()
        
        if "data structure" in name_lower or "algorithm" in name_lower:
            if "tree" in topic.lower():
                return f"Construct, step-by-step, a balanced binary search tree (BST) for the sequence {{45, 12, 89, 34, 70, 23, 9}}. Then, apply AVL rotation rules and show every single/double rotation (LL, RR, LR, RL) performed to maintain balance after inserting 55 and 61. Prove the final tree is an AVL Tree."
            elif "sort" in topic.lower():
                return f"Trace the Merge Sort and Quick Sort algorithms on the array [38, 27, 43, 3, 9, 82, 10]. Compare the best, average, and worst-case time complexity of both using the Master Theorem. Also, prove that comparison-based sorting cannot beat $\\Omega(n \\log n)$ in the worst case."
            elif "shortest path" in topic.lower() or "path" in topic.lower():
                return f"State Dijkstra's shortest path algorithm and trace it on a 6-node weighted directed graph, showing all relaxations. Why does it fail with negative edge weights? Replace it with Bellman-Ford and prove its correctness using induction."
            elif "dynamic" in topic.lower():
                return f"Bright them all: solve 0/1 Knapsack with W=[2,3,4,5], V=[3,4,5,6], C=8. Build the full 2D dynamic programming table, retrieve the optimal item subset, and compare the space-optimized 1D version."
            elif "hash" in topic.lower():
                return f"Apply separate chaining and linear probing to insert keys 27, 42, 91, 18, 52, 64 into a hash table of size 10 with hash function $h(k) = k \\mod 10$. Analyze the load factor and search performance."
            else:
                return f"Explain in detail the theory behind '{topic}' from {module_title}, supporting with appropriate examples, pseudo-code, and complexity analysis."
        
        elif "operating" in name_lower or "os" in name_lower:
            if "schedul" in topic.lower():
                return f"Draw Gantt Charts and calculate the average waiting & turnaround times for processes P1(AT=0, BT=8), P2(AT=1, BT=4), P3(AT=2, BT=9), P4(AT=3, BT=5) under: (a) FCFS, (b) SRTF, (c) Round Robin (Quantum=2). Identify which performs best and justify."
            elif "deadlock" in topic.lower() or "banker" in topic.lower():
                return f"Given 5 processes P0-P4 and 3 resource types A, B, C, apply Banker's Algorithm to determine if the system is in a safe state. Provide the safe sequence and prove it prevents deadlock."
            elif "semaphore" in topic.lower():
                return f"Explain the Producer-Consumer bounded-buffer problem and implement it using counting semaphore `empty=0, full=0, mutex=1`. Explain how the semaphore values change with each operator invocation. Analyze how starvation is prevented."
            elif "memory" in topic.lower() or "page" in topic.lower():
                return f"Page reference string: 7, 0, 1, 2, 0, 3, 0, 4, 2, 3, 0, 3, 2 = 13 references with 3 frames. Simulate FIFO, LRU, and Optimal page replacement and count page faults for each. Discuss Belady's Anomaly."
            else:
                return f"Explain the conceptual mechanisms, internals, and design trade-offs of '{topic}' from {module_title}, with examples."
        
        elif "database" in name_lower:
            if "normal" in topic.lower():
                return f"Given relation R(A, B, C, D, E) with functional dependencies F = {{A->B, BC->D, E->C, D->A}}: (a) Find all candidate keys. (b) Determine the highest normal form. (c) Decompose into BCNF losslessly while preserving dependencies."
            elif "transaction" in topic.lower() or "lock" in topic.lower():
                return f"Consider schedules S1 and S2 with transactions T1, T2, T3. Determine whether each is conflict-serializable using a precedence graph. Then compare Strict 2PL vs Timestamp Ordering Protocol."
            elif "sql" in topic.lower() or "relational" in topic.lower():
                return f"Given University database (Student, Course, Enrolls, Professor), write SQL and equivalent Relational Algebra for: (a) Find courses with more than 30 students, (b) List students taking all courses taught by 'Prof. Rao', (c) Create a view of top-10 GPAs with aggregate queries."
            elif "index" in topic.lower():
                return f"Build a B+ tree of order 3 by inserting keys {10, 20, 5, 6, 12, 30, 7, 17} one-by-one. Show leaf and internal node splits. Then analyze a range query performance on the built B+ tree."
            else:
                return f"Explain '{topic}' from {module_title}, including underlying structures and practical applications in SQL or ORMs."
        
        elif "network" in name_lower or "computer network" in name_lower:
            if "tcp" in topic.lower() or "transport" in topic.lower():
                return f"Describe the TCP three-way handshake and connection termination. Explain TCP flow control using sliding window and conflation of Slow Start, Congestion Avoidance, Fast Retransmit & Fast Recovery (AIMD)."
            elif "routing" in topic.lower() or "network layer" in topic.lower():
                return f"Design a network with 12 routers (typical campus topology) and apply OSPF and BGP intra/inter-domain routing. Show link-state packet floods and Dijkstra SPF trees. Compute best paths."
            elif "ip" in topic.lower():
                return f"Given an IPv4 address 192.168.10.0/22, (a) divide it into 4 subnets with 255 hosts each, (b) compute broadcast and network addresses, (c) explain subnetting & CIDR."
            elif "mac" in topic.lower() or "protocol" in topic.lower():
                return f"Explain CSMA/CD and CSMA/CA for LAN and WLAN MAC respectively. Show how binary exponential backoff works with a concrete collision scenario."
            else:
                return f"Analyze '{topic}' from {module_title} and apply it to a real network scenario with diagrams."
        
        elif "cryptograph" in name_lower or "security" in name_lower:
            if "aes" in topic.lower() or "symmetric" in topic.lower():
                return f"Describe AES-128 encryption in detail: SubBytes, ShiftRows, MixColumns, AddRoundKey for a single round, plus key expansion. Illustrate with a small block."
            elif "rsa" in topic.lower() or "public key" in topic.lower() or "asymmetric" in topic.lower():
                return f"Generate RSA keys with p=61, q=53. Compute n, phi(n), public key e, private key d. Encrypt the message M=65 and decrypt the ciphertext to verify correctness. Discuss computational security."
            elif "hash" in topic.lower():
                return f"Explain the properties of cryptographic hash functions (Preimage resistance, Second Preimage resistance, Collision resistance). Show how SHA-256 processes a block and explain Merkle-Damgard construction."
            else:
                return f"Analyze '{topic}' from {module_title} and explain its role in modern security protocols."
        
        elif "machine learning" in name_lower or "ai" in name_lower:
            return f"Analyze '{topic}' from {module_title}. Build an end-to-end appealing ML model (data preprocessing, feature engineering, model selection, evaluation) to demonstrate the concept with performance metrics."
        
        elif "forensic" in name_lower or "cyber law" in name_lower:
            return f"Analyze '{topic}' from {module_title} with respect to Indian & international legal frameworks and digital forensic methodologies."
        
        else:
            return f"Discuss the core principles of '{topic}' within {module_title}, including definitions, applications, and critical analysis."
    ...
    def search_online_pyq(
        ...
        for p in all_papers:
            sub_score = fast_fuzzy_similarity(subject, p["subject"])
            col_score = fast_fuzzy_similarity(college, p["college"]) if college else 1.0
            
            if (sub_score > 0.4 or subject.lower() in p["subject"].lower() or subject.lower() in p["title"].lower()) and \
               (col_score > 0.3 or not college or college.lower() in p["college"].lower()):
                p_copy = dict(p)
        ...
        try:
            query = f"{subject} university previous year question paper exam assignment"
            if college:
                query = f"{college} {query}"
            if year:
                query += f" {year}"
            ...
            if resp.status_code == 200:
                ...
                for res in web_results:
                    ...
                    results.append({
                        ...
                        "parsed_questions": [
                            {"num": 1, "marks": 10, "type": "Exam Question", "topic": subject, "text": snippet_text[:150] if snippet_text else f"Comprehensive past questions on {subject}"}
                        ],
                        "match_score": 0.85,
