...
async def startup_event():
    ...
    # Seed comprehensive curriculum and study content
    try:
        from .database_init import seed_all
        seed_all()
    except Exception as e:
        print(f"Curriculum seeding: {e}")
...
async def serve_index():
    ...
    return HTMLResponse(content="<h1>Assignment Sorter UI Loading...</h1>")
...
async def search_pyq(
    subject: str = Query(..., description="Subject or course name"),
    college: Optional[str] = Query(None),
    semester: Optional[str] = Query(None),
