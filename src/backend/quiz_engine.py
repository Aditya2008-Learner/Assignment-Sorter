...
"""
        ...
        all_qs = [
            {
                ...
                "question": "In an AVL tree, what is the maximum permissible difference between the heights of the left and right subtrees of any node?",
                "options": ["0", "1", "2", "log(n)"],
                "correct_index": 1,
                "explanation": "By definition, the balance factor BF = height(left) - height(right) in an AVL tree must be within {-1, 0, +1}, meaning a maximum difference of 1.",
            ...
            },
            {
                ...
                "correct_index": 1,
                "explanation": "Each vertex extraction takes $\\mathcal{O}(\\log V)$ (total $V \\log V$), and each edge relaxation can decrease key in $\\mathcal{O}(\\log V)$ (total $E \\log V$), yielding $\\mathcal{O}((V+E)\\log V)$.",
            ...
            },
            {
                ...
                "question": "What is the space complexity of the standard space-optimized 1D array approach for the 0/1 Knapsack problem with capacity $W$?",
                "options": [
                    "$\\mathcal{O}(N \\cdot W)$",
                    "$\\mathcal{O}(W)$",
                    "$\\mathcal{O}(N)$",
                ...
                "correct_index": 1,
                "explanation": "By iterating the capacity array backwards from $W$ down to item weight $w_i$, we only require a single 1D array of size $W+1$, reducing space from $\\mathcal{O}(N \\cdot W)$ to $\\mathcal{O}(W)$.",
                "bloom_level": "Apply"
            },
            {
                "id": "q5",
                ...
                "question": "A relation $R$ is in Boyce-Codd Normal Form (BCNF) if and only if for every non-trivial functional dependency $X \\to Y$:",
                "options": [
                    "$Y$ is a prime attribute",
                    "$X$ is a Super Key of $R$",
                    "$X$ contains at least one prime attribute",
                    "$R$ has no composite primary keys"
                ],
                "correct_index": 1,
                "explanation": "BCNF eliminates all redundancy based on functional dependencies by strictly mandating that the determinant $X$ must be a superkey for every non-trivial FD.",
                "bloom_level": "Analyze"
            }
        ]
    ...
    @staticmethod
    def grade_attempt(quiz_id: str, submitted_answers: List[Dict[str, Any]], time_taken: int = 60) -> Dict[str, Any]:
        ...
        for q in questions:
            ...
            sub = next((a for a in submitted_answers if str(a.get("question_id")) == str(qid)), None)
            user_choice = sub.get("selected_index") if sub else None
