...
"""
        ...
        if current_level == 1:
            ...
            if "master theorem" in q_lower:
                ...
                hint = "**Level 1 (Starting Direction):** Remember balance factor $BF = \\text{height}(left) - \\text{height}(right)$. Check if any node has $|BF| > 1$ after each insertion."
            ...
            hint = f"**Level 2 (Methodology & Formula):** Set up the formal governing equations. Substitute the given values into the formula and verify dimensional consistency."
            if "master theorem" in q_lower:
                hint = "**Level 2 (Methodology & Formula):** If $f(n) = \\Theta(n^{\\log_b a})$, then $T(n) = \\Theta(n^{\\log_b a} \\log n)$ (Case 2). If $f(n)$ grows polynomially faster or slower, use Case 3 or Case 1 respectively."
            elif "avl" in q_lower:
                hint = "**Level 2 (Methodology & Formula):** Identify the rotation type: Left-Left requires Right rotation; Right-Right requires Left rotation; Left-Right requires Left child rotate then Right parent rotate."
            elif "knapsack" in q_lower:
                hint = "**Level 2 (Methodology & Formula):** Initialize a table of size $(N+1) \\times (C+1)$ with zeros. Fill row by row from $i=1 \\dots N$ and $w=0 \\dots C$."
        else:
...
4. **Final Complexity:** $T(n) = \\Theta(n^2 \\log n)$."""
        ...
        return {
            "level": current_level,
            "hint_text": hint,
            "model": "Academic Socratic Engine"
        }
