...
const viewTitles = {
  ...
  window.studyTopic = async function(courseCode, courseName, moduleTopics) {
    showToast('Opening study materials for ' + courseName, 'info');
    // Switch to notebook view with course context
    switchView('notebook');
    // Pre-fill the notes generation
    setTimeout(() => {
      const subjectSel = $('#notes-notebook-select');
      const topicInp = $('#notes-topic-input');
      if (topicInp) topicInp.value = courseName;
    }, 300);
  };
